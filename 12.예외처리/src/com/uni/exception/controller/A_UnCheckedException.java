...
public void method3() {
	try {
		// - ClassCastException : 허용할 수 없는 형변환이 진행되는 예외 상황 --> 개발자가 실수 (코드 해결하면됨)
		Object obj = 'a'; // char -> (autoBoxing) -> Character -> (
	...
	/*} catch(ClassCastException e) {
		...
		System.out.println("객체 없이 메소드 사용 불가");
	}*/
		// Multi catch문 --> 나란히 작성하는 경우 형제레벨만 함께 사용
	} catch(ClassCastException|ArrayIndexOutOfBoundsException|NullPointerException e) {
		e.printStackTrace(); // 유형 + 에러 발생 경로 + 에러 메시지
		
		System.out.println(e.getMessage()); // 에러 메시지
		System.out.println(e.toString()); // 유형 + 에러 메시지
